...
The game is built with [boardgame.io](https://github.com/boardgameio/boardgame.io), a framework to build turn-based games, and is wrapped into a [Next.js](https://nextjs.org/) application that uses [Tailwind](https://github.com/tailwindlabs/tailwindcss) for styling and [Framer Motion](https://github.com/framer/motion) for animations. The code contains many artefacts betraying its prototype nature — the code quality and organisation will reflect that.
...
In the crew phase, every player takes turn to do a few actions. However, they first have to decide whether to open the comms (allowing them to chat with other players about strategy) or close them (where they have to make their decisions on their own). If they choose not to open the comms, they get an upgrade. This is to help mitigate quarterbacking but the not talking part isn't enforced through the game. Then the active player can choose to do a few things: move to another room, use a skill (hack, lockpick or neutralise), get an upgrade, request an upgrade from another player or use an upgrade.
...
If `skipLobby` is turned on, the game starts straight away with default characters and skips the introduction to the story. If `skipEncounter` is on, the encounter phase where you have to scratch to reveal what negative thing happens to your character is skipped. `skipBlockScreen` means the blocking overlay usually displayed during an important action will not appear (any move you make will stop the animation and get to the next state immediately). `boostCharacters` gives you overpowered characters meaning you are less likely to fail skill tests. When `showMap` is on, the whole map is visible even if not explored (useful when developing and trying out new maps).
...
Thanks to everyone who took part in this project and helped us refine what we think is a fun game and a great experiment. Thanks particularly to our friends who reviewed the [white paper](https://rig.rd.labs.bbc/), took part in the playtesting and provided invaluable feedback and support: Florian Bœuf, Dr Elsa Call, Dr Maxime Courcelle, Sébastien Gatty, Dr Alana Laing and Dr Cécile Triay.

The background music is sourced from royalty-free music made available on Youtube by Wee Free Music (https://youtu.be/rpMvf4Gdn-4 and https://youtu.be/a6v-RL5ypR4). The spot sounds are sourced from royalty-free effects provided by [Sonniss](https://sonniss.com/gameaudiogdc) — they were cut and mixed as needed.

The character illustrations were drawn by [Rob Turpin](https://www.instagram.com/thisnorthernboy/). All other illustrations were adapted from patent drawings. Circuit board pattern modified and adapted from [Pixabay](https://pixabay.com/vectors/computer-cyber-circuitry-circuits-3163436/).

Icons are original except [Fanned cards](https://github.com/Templarian/MaterialDesign), [Sound on/off](https://github.com/vmware/clarity), [Chip](https://github.com/ionic-team/ionicons), [Lock](https://github.com/atisawd/boxicons), [Heart](https://github.com/twbs/icons). These icons are included and redistributed under their respective licences.
 
Apart from the "story" and the third party icons listed above, the Identical game is licensed under the open source [BSD-3-Clause](/LICENSE) licence. The "story" means the game narrative as expressed as text in [source code](/game/story.js), viewed by the user as text and played out as audio; including the characters in the story and their [names, description](/game/Characters.js) and [images](/public/static/images/players/). The "story" is licensed under the [Creative Commons Non-Commercial 4.0](https://creativecommons.org/licenses/by-nc/4.0/) licence.
