...
const App = () => {
			...
			const unsub = gameClient.subscribe((state) => {
				if (state !== null) {
					if (playerInfo) {
						...
						router.replace(`/play/${matchID}/${playerInfo.playerID}`)
					}
					else {
	...
	return <div className='w-full relative h-full'>
		<GlitchCanvas />
		<div className='relative text-white w-10/12 max-w-md mx-auto space-y-8 pt-8'>
			<h1 className='text-2xl tracking-wider uppercase text-neutralise'>Join the adventure</h1>
